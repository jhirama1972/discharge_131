...
char speed_str[8];
char rpm_str[8];
float roller_size = 18.90;
float wheel_size = 47.8;
float roller_hole = 1 ;
/*
*/
void rpm_count() {
  Serial.println("input");
  if ( digitalRead(RPM_in) == LOW ) {
    rpm_cnt++;
  }
}
...
void calc_rpm() {
  ...
  f_tire = 3.14159 * roller_size ;
  f_rpm = (float)(60000000) / float((elapsed_time / rpm_cnt));
  f_rpm /= roller_hole;
  f_rpm = f_rpm / (float)(wheel_size / roller_size); // for ラジコンPRM用　演算
  spnum = (f_tire * 60.0 * ((float)60 / (elapsed_time / rpm_cnt)));
  spnum /= roller_hole;
  speed_num = spnum ;
  rpm_cnt = 0;
  if ( max_num < speed_num ) {
    max_num = speed_num;
  }
  start_time = micros();
  interrupts();
...
#if MULTI_CHECK == true
  attachInterrupt(digitalPinToInterrupt(RPM_in), rpm_count, FALLING );
#endif
}
...
void timer_1sec() {
  ...
  //  Serial.println(sw_stat[0]);
  //  Serial.println(sw_stat2[0]);
...
#if MULTI_CHECK == true
    // アナログ入力の値を電圧に変換(mV)
    g_in = analogRead(GAUSS_in);
    if ( g_in >= 100 && g_in <= 924 ) {
      read_gauss = g_in;
      f_gauss += read_gauss;
      gcnt ++;
    }
...
// 3.0
      ...
      if ( sw_stat[0] & SW_ENTER_BIT) {
        ...
        if ( display_stat == (FUNC_DISCHARGE | FUNC_EDIT) || display_stat == (FUNC_DISCHARGE2 | FUNC_EDIT)) {
          ...
          display_stat &= ~FUNC_EDIT2;
          display_stat |= FUNC_MOVE;
          discharge_check();     // このなかで放電のduty決まっている。
          if ( shibori == 1 ) {
